## 更新说明
- 2026-09-09 Version 1.7.3

       * `/status` 的「重新加载全部配置」现在也能应用 rc：先在子进程用 `-check-config` 校验，rc 有语法错误时指出位置并继续使用当前配置；校验通过且 rc 确有改动时自动重启（Windows 改为退出待手动启动）。rc 未改动时只就地重载规则文件，不重启
       * 新增 `-check-config` 命令行开关：解析配置、报告问题后退出，不启动代理

- 2026-09-09 Version 1.7.2
