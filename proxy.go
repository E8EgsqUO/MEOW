...
// very long time.
const connectBufSize = 32 * 1024

// Larger buffers reduce read/write system calls for high-throughput tunnels.
// The sync.Pool releases excess buffers to the runtime under memory pressure.
var connectBuf = newByteBufferPool(connectBufSize)
...
func copyClient2Server(c *clientConn, sv *serverConn, r *Request) (err error) {
	...
	for {
		...
		if err = writeFull(sv, buf[:n]); err != nil {
