package main

import (
	"bufio"
	"bytes"
	"context"
	"crypto/tls"
	"errors"
	"fmt"
	"io"
	"math"
	"net"
	"net/netip"
	"os"
	"strconv"
	"strings"
	"sync"
	"syscall"
	"time"

	ss "github.com/shadowsocks/shadowsocks-go/shadowsocks"
)

// As I'm using ReadSlice to read line, it's possible to get
// bufio.ErrBufferFull while reading line, so set it to a large value to
// prevent such problems.
//
// For limits about URL and HTTP header size, refer to:
// http://stackoverflow.com/questions/417142/what-is-the-maximum-length-of-a-url
// "de facto limit of 2000 characters"
// http://www.mnot.net/blog/2011/07/11/what_proxies_must_do
// "URIs should be allowed at least 8000 octets, and HTTP headers should have
// 4000 as an absolute minimum".
// In practice, there are sites using cookies larger than 4096 bytes,
// e.g. www.fitbit.com. So set http buffer size to 8192 to be safe.
const httpBufSize = 8192

var (
	httpBuf        = newByteBufferPool(httpBufSize)
	httpReaderPool = newBufferedReaderPool(httpBufSize)
)

// If no keep-alive header in response, use this as the keep-alive value.
const defaultServerConnTimeout = 5 * time.Second

// Close client connection if no new requests received in some time.
// (On OS X, the default soft limit of open file descriptor is 256, which is
// very conservative and easy to cause problem if we are not careful to limit
// open fds.)
const clientConnTimeout = 5 * time.Second
const fullKeepAliveHeader = "Keep-Alive: timeout=5\r\n"

// Some code are learnt from the http package

var zeroTime time.Time

type directConn struct {
	net.Conn
}

func (dc directConn) String() string {
	return "direct connection"
}

type serverConn struct {
	net.Conn
	bufRd              *bufio.Reader
	hostPort           string
	fallbackHost       string // pending route learning, until the target responds
	willCloseOn        time.Time
	readTimeout        time.Duration
	closeOnce          sync.Once
	closeErr           error
	transportCloseOnce sync.Once
	transportCloseErr  error
}

type clientConn struct {
	net.Conn  // connection to the proxy client
	bufRd     *bufio.Reader
	proxy     Proxy
	ctx       context.Context
	closeOnce sync.Once
}

var (
	errPageSent      = errors.New("error page has sent")
	errClientTimeout = errors.New("read client request timeout")
	errAuthRequired  = errors.New("authentication requried")
)

type Proxy interface {
	Serve(context.Context, *sync.WaitGroup)
	Addr() string
	genConfig() string // for upgrading config
}

var listenProxy []Proxy

func addListenProxy(p Proxy) {
	listenProxy = append(listenProxy, p)
}

type httpProxy struct {
	addr      string // listen address, contains port
	port      string // for use when generating PAC
	addrInPAC string // proxy server address to use in PAC
	proto     string
}

func newHttpProxy(addr, addrInPAC string, proto string) *httpProxy {
	_, port, err := net.SplitHostPort(addr)
	if err != nil {
		panic("proxy addr" + err.Error())
	}
	return &httpProxy{addr, port, addrInPAC, proto}
}

func (proxy *httpProxy) genConfig() string {
	if proxy.addrInPAC != "" {
		return fmt.Sprintf("listen = %s://%s %s", proxy.proto, proxy.addr, proxy.addrInPAC)
	} else {
		return fmt.Sprintf("listen = %s://%s", proxy.proto, proxy.addr)
	}
}

func (proxy *httpProxy) Addr() string {
	return proxy.addr
}

func (hp *httpProxy) Serve(ctx context.Context, wg *sync.WaitGroup) {
	var err error
	var ln net.Listener
	var clients sync.WaitGroup
	defer func() {
		clients.Wait()
		wg.Done()
	}()

	if hp.proto == "https" {
		cert, err := tls.LoadX509KeyPair(config.Cert, config.Key)
		if err != nil {
			criticalf("HTTPS listener certificate error: %v", err)
			return
		}
		config := tls.Config{
			Certificates:       []tls.Certificate{cert},
			InsecureSkipVerify: false,
			MinVersion:         tls.VersionTLS12,
		}
		ln, err = tls.Listen("tcp", hp.addr, &config)
	} else {
		ln, err = net.Listen("tcp", hp.addr)
	}

	if err != nil {
		criticalf("listen %s %s failed: %v", hp.proto, hp.addr, err)
		return
	}
	go func() {
		<-ctx.Done()
		_ = ln.Close()
	}()
	host, _, _ := net.SplitHostPort(hp.addr)
	var pacURL string
	if host == "" || host == "0.0.0.0" || host == "::" {
		pacURL = fmt.Sprintf("%s://<hostip>:%s/pac", hp.proto, hp.port)
	} else if hp.addrInPAC == "" {
		pacURL = fmt.Sprintf("%s://%s/pac", hp.proto, hp.addr)
	} else {
		pacURL = fmt.Sprintf("%s://%s/pac", hp.proto, hp.addrInPAC)
	}
	info.Printf("listen %s %s, PAC url %s\n", hp.proto, hp.addr, pacURL)

	for {
		conn, err := ln.Accept()
		if err != nil {
			if ctx.Err() != nil {
				return
			}
			errl.Printf("%s proxy(%s) accept %v\n", hp.proto, ln.Addr(), err)
			if isErrTooManyOpenFd(err) {
				connPool.CloseAll()
			}
			time.Sleep(time.Millisecond)
			continue
		}
		c := newClientConn(ctx, conn, hp)
		clients.Add(1)
		go func() {
			defer clients.Done()
			serveClient(ctx, c)
		}()
	}
}

type meowProxy struct {
	addr   string
	method string
	passwd string
	cipher *ss.Cipher
}

func newMeowProxy(method, passwd, addr string) *meowProxy {
	cipher, err := ss.NewCipher(method, passwd)
	if err != nil {
		Fatal("can't initialize meow proxy server", err)
	}
	return &meowProxy{addr, method, passwd, cipher}
}

func (cp *meowProxy) genConfig() string {
	method := cp.method
	if method == "" {
		method = "table"
	}
	return fmt.Sprintf("listen = meow://%s:%s@%s", method, cp.passwd, cp.addr)
}

func (cp *meowProxy) Addr() string {
	return cp.addr
}

func (cp *meowProxy) Serve(ctx context.Context, wg *sync.WaitGroup) {
	var clients sync.WaitGroup
	defer func() {
		clients.Wait()
		wg.Done()
	}()
	ln, err := net.Listen("tcp", cp.addr)
	if err != nil {
		criticalf("listen meow %s failed: %v", cp.addr, err)
		return
	}
	go func() {
		<-ctx.Done()
		_ = ln.Close()
	}()
	info.Printf("meow proxy address %s\n", cp.addr)

	for {
		conn, err := ln.Accept()
		if err != nil {
			if ctx.Err() != nil {
				return
			}
			errl.Printf("meow proxy(%s) accept %v\n", ln.Addr(), err)
			if isErrTooManyOpenFd(err) {
				connPool.CloseAll()
			}
			time.Sleep(time.Millisecond)
			continue
		}
		c := newClientConn(ctx, ssConn{ss.NewConn(conn, cp.cipher.Copy())}, cp)
		clients.Add(1)
		go func() {
			defer clients.Done()
			serveClient(ctx, c)
		}()
	}
}

func serveClient(ctx context.Context, c *clientConn) {
	done := make(chan struct{})
	go func() {
		select {
		case <-ctx.Done():
			_ = c.Conn.Close()
		case <-done:
		}
	}()
	c.serve()
	close(done)
}

func newClientConn(ctx context.Context, cli net.Conn, proxy Proxy) *clientConn {
	c := &clientConn{
		Conn:  cli,
		bufRd: httpReaderPool.Get(cli),
		proxy: proxy,
		ctx:   ctx,
	}
	if debug {
		debug.Printf("cli(%s) connected, total %d clients\n",
			cli.RemoteAddr(), incCliCnt())
	}
	return c
}

func (c *clientConn) releaseBuf() {
	if c.bufRd != nil {
		// debug.Println("release client buffer")
		httpReaderPool.Put(c.bufRd)
		c.bufRd = nil
	}
}

func (c *clientConn) Close() {
	c.closeOnce.Do(func() {
		c.releaseBuf()
		if debug {
			debug.Printf("cli(%s) closed, total %d clients\n",
				c.RemoteAddr(), decCliCnt())
		}
		_ = c.Conn.Close()
	})
}

func (c *clientConn) setReadTimeout(msg string) {
	// Always keep connections alive for meow conn from client for more reuse.
	// For other client connections, set read timeout so we can close the
	// connection after a period of idle to reduce number of open connections.
	if _, ok := c.Conn.(ssConn); !ok {
		// make actual timeout a little longer than keep-alive value sent to client
		setConnReadTimeout(c.Conn, clientConnTimeout+2*time.Second, msg)
	}
}

func (c *clientConn) unsetReadTimeout(msg string) {
	if _, ok := c.Conn.(ssConn); !ok {
		unsetConnReadTimeout(c.Conn, msg)
	}
}

func setConnReadTimeout(cn net.Conn, d time.Duration, msg string) {
	if err := cn.SetReadDeadline(time.Now().Add(d)); err != nil {
		errl.Println("set readtimeout:", msg, err)
	}
}

func unsetConnReadTimeout(cn net.Conn, msg string) {
	if err := cn.SetReadDeadline(zeroTime); err != nil {
		// It's possible that conn has been closed, so use debug log.
		debug.Println("unset readtimeout:", msg, err)
	}
}

// Listen address as key, not including port part.
var selfListenAddr map[string]bool

// Called in main, so no need to protect concurrent initialization.
func initSelfListenAddr() {
	selfListenAddr = make(map[string]bool)
	// Add empty host to self listen addr, in case there's no Host header.
	selfListenAddr[""] = true
	for _, proxy := range listenProxy {
		addr := proxy.Addr()
		// Handle wildcard address.
		if addr[0] == ':' || strings.HasPrefix(addr, "0.0.0.0") || strings.HasPrefix(addr, "[::]") {
			for _, ad := range hostAddr() {
				selfListenAddr[ad] = true
			}
			selfListenAddr["localhost"] = true
			selfListenAddr["127.0.0.1"] = true
			continue
		}

		host, _, err := net.SplitHostPort(addr)
		if err != nil {
			panic("listen addr invalid: " + addr)
		}
		selfListenAddr[host] = true
		if host == "127.0.0.1" {
			selfListenAddr["localhost"] = true
		} else if host == "localhost" {
			selfListenAddr["127.0.0.1"] = true
		}
	}
}

func isSelfRequest(r *Request) bool {
	if r.URL.HostPort != "" {
		if !isStatusPath(r.URL.Path) && r.URL.Path != "/pac" && !strings.HasPrefix(r.URL.Path, "/pac?") {
			return false
		}
		if !selfListenAddr[r.URL.Host] {
			return false
		}
		for _, proxy := range listenProxy {
			_, port, err := net.SplitHostPort(proxy.Addr())
			if err == nil && port == r.URL.Port {
				return true
			}
		}
		return false
	}
	// Maxthon sometimes sends requests without host in request line,
	// in that case, get host information from Host header.
	// But if client PAC setting is using meow server's DNS name, we can't
	// decide if the request is for meow itself (need reverse lookup).
	// So if request path seems like getting PAC, simply return true.
	if r.URL.Path == "/pac" || strings.HasPrefix(r.URL.Path, "/pac?") || isStatusPath(r.URL.Path) {
		return true
	}
	r.URL.ParseHostPort(r.Header.Host)
	if selfListenAddr[r.URL.Host] {
		return true
	}
	debug.Printf("fixed request with no host in request line %s\n", r)
	return false
}

func (c *clientConn) serveSelfURL(r *Request) (err error) {
	if _, ok := c.proxy.(*httpProxy); !ok {
		goto end
	}
	if r.Method != "GET" {
		goto end
	}
	if isStatusPath(r.URL.Path) {
		return sendRuntimeStatus(c, r)
	}
	if r.URL.Path == "/pac" || strings.HasPrefix(r.URL.Path, "/pac?") {
		sendPAC(c)
		// PAC header contains connection close, send non nil error to close
		// client connection.
		return errPageSent
	}
end:
	sendErrorPage(c, "404 not found", "Page not found",
		genErrMsg(r, nil, "Serving request to meow proxy."))
	errl.Printf("cli(%s) page not found, serving request to meow %s\n%s",
		c.RemoteAddr(), r, r.Verbose())
	return errPageSent
}

func dbgPrintRq(c *clientConn, r *Request, direct bool) {
	if r.Trailer {
		errl.Printf("cli(%s) request  %s has Trailer header\n%s",
			c.RemoteAddr(), r, r.Verbose())
	}
	if dbgRq {
		var connType string
		if direct {
			connType = "DIRECT"
		} else {
			connType = "PROXY"
		}
		if verbose {
			dbgRq.Printf("%s %s %s\n%s\n", connType, c.RemoteAddr(), r, r.Verbose())
		} else {
			dbgRq.Printf("%s %s %s\n", connType, c.RemoteAddr(), r)
		}
	}
}

func (c *clientConn) serve() {
	var r Request
	var rp Response
	var sv *serverConn
	var err error

	var authed bool
	// For meow proxy server, authentication is done by matching password.
	if _, ok := c.proxy.(*meowProxy); ok {
		authed = true
	}

	defer func() {
		r.releaseBuf()
		c.Close()
	}()

	// Refer to implementation.md for the design choices on parsing the request
	// and response.
	for {
		if c.bufRd == nil {
			panic("client read buffer nil")
		}

		if err = parseRequest(c, &r); err != nil {
			debug.Printf("cli(%s) parse request %v\n", c.RemoteAddr(), err)
			if err == io.EOF || isErrConnReset(err) {
				return
			}
			if err != errClientTimeout {
				sendErrorPage(c, "404 Bad request", "Bad request", err.Error())
				return
			}
			sendErrorPage(c, statusRequestTimeout, statusRequestTimeout,
				"Your browser didn't send a complete request in time.")
			return
		}

		// PAC may leak frequently visited sites information. But if meow
		// requires authentication for PAC, some clients may not be able
		// handle it. (e.g. Proxy SwitchySharp extension on Chrome.)
		if isSelfRequest(&r) {
			if err = c.serveSelfURL(&r); err != nil {
				return
			}
			continue
		}

		if auth.required && !authed {
			if err = Authenticate(c, &r); err != nil {
				errl.Printf("cli(%s) %v\n", c.RemoteAddr(), err)
				// Request may have body. To make things simple, close
				// connection so we don't need to skip request body before
				// reading the next request.
				return
			}
			authed = true
		}

		if sv, err = c.getServerConn(&r); err != nil {
			if debug {
				debug.Printf("cli(%s) failed to get server conn %v\n", c.RemoteAddr(), &r)
			}
			// Failed connection will send error page back to the client.
			// For CONNECT, the client read buffer is released in copyClient2Server,
			// so can't go back to getRequest.
			if err == errPageSent && !r.isConnect {
				if r.hasBody() {
					// skip request body
					debug.Printf("cli(%s) skip request body %v\n", c.RemoteAddr(), &r)
					sendBody(io.Discard, c.bufRd, r.ContLen, r.Chunking)
				}
				continue
			}
			return
		}

		if r.isConnect {
			// server connection will be closed in doConnect
			err = sv.doConnect(&r, c)
			// debug.Printf("doConnect %s to %s done\n", c.RemoteAddr(), r.URL.HostPort)
			return
		}

		if err = sv.doRequest(c, &r, &rp); err != nil {
			// For client I/O error, we can actually put server connection to
			// pool. But let's make thing simple for now.
			sv.Close()
			if err == errPageSent && (!r.hasBody() || r.hasSent()) {
				// Can only continue if request has no body, or request body
				// has been read.
				continue
			}
			return
		}
		// Put server connection to pool, so other clients can use it.
		_, isMeowConn := sv.Conn.(meowConn)
		if rp.ConnectionKeepAlive || isMeowConn {
			if debug {
				debug.Printf("cli(%s) connPool put %s", c.RemoteAddr(), sv.hostPort)
			}
			// If the server connection is not going to be used soon,
			// release buffer before putting back to pool can save memory.
			sv.releaseBuf()
			connPool.Put(sv)
		} else {
			if debug {
				debug.Printf("cli(%s) server %s close conn\n", c.RemoteAddr(), sv.hostPort)
			}
			sv.Close()
		}

		if !r.ConnectionKeepAlive {
			if debug {
				debug.Printf("cli(%s) close connection\n", c.RemoteAddr())
			}
			return
		}
	}
}

func genErrMsg(r *Request, sv *serverConn, what string) string {
	if sv == nil {
		return fmt.Sprintf("<p>HTTP Request <strong>%v</strong></p> <p>%s</p>", r, what)
	}
	return fmt.Sprintf("<p>HTTP Request <strong>%v</strong></p> <p>%s</p> <p>Using %s.</p>",
		r, what, sv.Conn)
}

func (c *clientConn) handleServerReadError(r *Request, sv *serverConn, err error, msg string) error {
	if debug {
		debug.Printf("cli(%s) server read error %s %T %v %v\n",
			c.RemoteAddr(), msg, err, err, r)
	}
	if err == io.EOF {
		return err
	}
	if isErrTimeout(err) || isErrConnReset(err) || isHttpErrCode(err) {
		return err
	}
	if r.responseNotSent() {
		sendErrorPage(c, "502 read error", err.Error(), genErrMsg(r, sv, msg))
		return errPageSent
	}
	errl.Printf("cli(%s) unhandled server read error %s %v %s\n", c.RemoteAddr(), msg, err, r)
	return err
}

func dbgPrintRep(c *clientConn, r *Request, rp *Response) {
	if rp.Trailer {
		errl.Printf("cli(%s) response %s has Trailer header\n%s",
			c.RemoteAddr(), rp, rp.Verbose())
	}
	if dbgRep {
		if verbose {
			dbgRep.Printf("%s %s\n%s", r, rp, rp.Verbose())
		} else {
			dbgRep.Printf("%s %s\n", r, rp)
		}
	}
}

func (c *clientConn) readResponse(sv *serverConn, r *Request, rp *Response) (err error) {
	sv.readTimeout = config.ReadTimeout
	defer func() {
		sv.readTimeout = 0
		_ = sv.SetReadDeadline(zeroTime)
	}()
	sv.initBuf()
	defer func() {
		rp.releaseBuf()
	}()

	if err = parseResponse(sv, r, rp); err != nil {
		sv.finishFallback(false)
		return c.handleServerReadError(r, sv, err, "parse response")
	}
	// Error responses may be generated by the proxy itself. Conservatively
	// learn only successful responses and redirects, not proxy error pages.
	sv.finishFallback(rp.Status >= 200 && rp.Status < 400)
	return c.forwardResponse(sv, r, rp)
}

// forwardResponse relays an already-parsed response and its body to the client.
func (c *clientConn) forwardResponse(sv *serverConn, r *Request, rp *Response) (err error) {
	dbgPrintRep(c, r, rp)
	r.state = rsRecvBody
	r.releaseBuf()

	if err = writeFull(c, rp.rawResponse()); err != nil {
		return err
	}

	rp.releaseBuf()

	if rp.hasBody(r.Method) {
		if err = sendBody(fullWriter{c}, sv.bufRd, rp.ContLen, rp.Chunking); err != nil {
			if debug {
				debug.Printf("cli(%s) send body %v\n", c.RemoteAddr(), err)
			}
			// Non persistent connection will return nil upon successful response reading
			if err == io.EOF {
				// For persistent connection, EOF from server is error.
				// Response header has been read, server using persistent
				// connection indicates the end of response and proxy should
				// not got EOF while reading response.
				// The client connection will be closed to indicate this error.
				// Proxy can't send error page here because response header has
				// been sent.
				return fmt.Errorf("read response body unexpected EOF %v", rp)
			} else if isErrOpRead(err) {
				return c.handleServerReadError(r, sv, err, "read response body")
			}
			// errl.Printf("cli(%s) sendBody error %T %v %v", err, err, r)
			return err
		}
	}
	r.state = rsDone
	if rp.ConnectionKeepAlive {
		if rp.KeepAlive == time.Duration(0) {
			sv.willCloseOn = time.Now().Add(defaultServerConnTimeout)
		} else {
			// debug.Printf("cli(%s) server %s keep-alive %v\n", c.RemoteAddr(), sv.hostPort, rp.KeepAlive)
			sv.willCloseOn = time.Now().Add(rp.KeepAlive)
		}
	}
	return
}

const statusCodeContinue = 100

// errExpectRejected is returned when the origin answered an "Expect:
// 100-continue" request with a final response before the body was sent. The
// response has already been relayed; the connection must be closed because the
// client may still transmit the body it announced.
var errExpectRejected = errors.New("origin rejected 100-continue request")

// relayExpectContinue relays the origin's interim decision for a request that
// carried "Expect: 100-continue". It returns proceed=true once a 100 response
// has been forwarded to the client and the body should now be streamed.
func (c *clientConn) relayExpectContinue(sv *serverConn, r *Request, rp *Response) (err error) {
	sv.readTimeout = config.ReadTimeout
	defer func() {
		sv.readTimeout = 0
		_ = sv.SetReadDeadline(zeroTime)
	}()
	sv.initBuf()

	const maxInterim = 10
	for i := 0; i < maxInterim; i++ {
		if err = parseResponseOnce(sv, r, rp); err != nil {
			sv.finishFallback(false)
			return c.handleServerReadError(r, sv, err, "parse interim response")
		}
		if rp.Status == statusCodeContinue {
			return writeFull(c, []byte("HTTP/1.1 100 Continue\r\n\r\n"))
		}
		if rp.Status > 100 && rp.Status < 200 && rp.Status != 101 {
			debug.Printf("ignore informational response %d for %v\n", rp.Status, r)
			continue
		}
		// A final response arrived instead of 100: relay it and stop.
		sv.finishFallback(rp.Status >= 200 && rp.Status < 400)
		if err = c.forwardResponse(sv, r, rp); err != nil {
			return err
		}
		return errExpectRejected
	}
	return errors.New("too many informational HTTP responses")
}

func (c *clientConn) getServerConn(r *Request) (*serverConn, error) {
	domainType := router.Route(c.ctx, r.URL, RouteOptions{
		ParentAvailable: !parentProxy.empty(),
		JudgeByIP:       config.JudgeByIP,
		IPv6:            config.IPv6Policy,
		TrustedDNS:      trustedResolver != nil,
		DNSVerify:       config.DNSVerify,
	})
	recordRouteStatus(r.URL.Host, domainType)
	// For CONNECT method, always create new connection.
	direct := (domainType == domainTypeDirect)
	if domainType == domainTypeReject {
		dbgRq.Printf("%s REJECT\n", r.URL)
		return nil, errors.New("Reject")
	}

	if r.isConnect {
		return c.createServerConn(r, direct)
	}
	sv := connPool.Get(r.URL.HostPort, direct)
	if sv != nil {
		if debug {
			debug.Printf("cli(%s) connPool get %s\n", c.RemoteAddr(), r.URL.HostPort)
		}
		return sv, nil
	}
	if debug {
		debug.Printf("cli(%s) connPool no conn %s", c.RemoteAddr(), r.URL.HostPort)
	}
	return c.createServerConn(r, direct)
}

func connectDirect2(ctx context.Context, url *URL, recursive bool) (net.Conn, error) {
	var c net.Conn
	var err error
	c, err = dialContext(ctx, "tcp", url.HostPort)
	if err != nil {
		debug.Printf("error direct connect to: %s %v\n", url.HostPort, err)
		if isErrTooManyOpenFd(err) && !recursive {
			return connectDirect2(ctx, url, true)
		}
		return nil, err
	}
	// debug.Println("directly connected to", url.HostPort)
	return directConn{c}, nil
}

func connectDirect(ctx context.Context, url *URL) (net.Conn, error) {
	return connectDirect2(ctx, url, false)
}

func isErrTimeout(err error) bool {
	if ne, ok := err.(net.Error); ok {
		return ne.Timeout()
	}
	return false
}

func isHttpErrCode(err error) bool {
	if config.HttpErrorCode <= 0 {
		return false
	}
	if err == CustomHttpErr {
		return true
	}
	return false
}

func isErrTooManyOpenFd(err error) bool {
	if ne, ok := err.(*net.OpError); ok {
		if se, seok := ne.Err.(*os.SyscallError); seok && (se.Err == syscall.EMFILE || se.Err == syscall.ENFILE) {
			errl.Println("too many open fd")
			return true
		}
	}
	return false
}

// canFallBackToParent reports whether a failed direct connection to url is
// worth retrying through the parent proxy.
func canFallBackToParent(url *URL) bool {
	if !config.DirectFallback || parentProxy.empty() {
		return false
	}
	// A local destination that will not answer is down, not blocked. Retrying
	// through the parent cannot help, and it would hand an internal address to
	// a remote server. url.Domain is empty for a bare hostname or a private
	// address, which is the same set the router sends direct unconditionally.
	if url.Domain == "" {
		return false
	}
	if addr, err := netip.ParseAddr(url.Host); err == nil {
		return !addrIsLocal(addr.Unmap())
	}
	return true
}

// fallBackToParent retries a failed direct connection through the parent proxy.
//
// A direct attempt that fails is the clearest evidence MEOW gets that its
// routing verdict was wrong: a stale address table, a poisoned DNS answer or a
// site blocked since the table was built all surface here, as a timeout or a
// reset. Nothing has been written to the server at this point, so the retry
// costs one connection and cannot duplicate a request.
//
// Connecting to the parent alone does not prove the target is reachable.
// Defer learning until an HTTP response or tunnel response confirms it.
func (c *clientConn) fallBackToParent(r *Request, directErr error) (net.Conn, error) {
	if !canFallBackToParent(r.URL) {
		return nil, directErr
	}
	if debug {
		debug.Printf("cli(%s) direct connection to %s failed (%v), retrying through parent proxy\n",
			c.RemoteAddr(), r.URL.HostPort, directErr)
	}
	srvconn, err := parentProxy.connect(c.ctx, r.URL)
	if err != nil {
		if !confirmsParentAvailable(err) {
			recordRuntimeError(r.URL.HostPort, err)
		}
		if debug {
			debug.Printf("cli(%s) parent proxy also failed for %s: %v\n",
				c.RemoteAddr(), r.URL.HostPort, err)
		}
		return nil, directErr
	}
	recordRouteStatusNamed(r.URL.Host, "PROXY (fallback)", domainTypeProxy)
	r.fellBackToParent = true
	dbgRq.Printf("%s direct failed, now using parent proxy\n", r.URL)
	return srvconn, nil
}

// MEOW !!!
// Connect to requested server according to whether it's visit count.
// If direct connection fails, try parent proxies.
func (c *clientConn) connect(r *Request, direct bool) (srvconn net.Conn, err error) {
	r.fellBackToParent = false
	var errMsg string

	if direct {
		dbgPrintRq(c, r, true)
		if srvconn, err = connectDirect(c.ctx, r.URL); err == nil {
			return
		}
		directErr := err
		if srvconn, err = c.fallBackToParent(r, directErr); err == nil {
			return
		}
		err = directErr
		errMsg = genErrMsg(r, nil, "Direct connection failed.")
		goto fail
	}

	if parentProxy.empty() {
		errMsg = genErrMsg(r, nil, "No parent proxy.")
		goto fail
	}

	// “我向来是不惮以最坏的恶意来揣测中国人的”
	dbgPrintRq(c, r, false)
	if srvconn, err = parentProxy.connect(c.ctx, r.URL); err == nil {
		return
	}
	if !confirmsParentAvailable(err) {
		recordRuntimeError(r.URL.HostPort, err)
	}
	errMsg = genErrMsg(r, nil, "Parent proxy connection failed.")

fail:
	sendErrorPage(c, "504 Connection failed", err.Error(), errMsg)
	return nil, errPageSent
}

func (c *clientConn) createServerConn(r *Request, direct bool) (*serverConn, error) {
	srvconn, err := c.connect(r, direct)
	if err != nil {
		return nil, err
	}
	sv := newServerConn(srvconn, r.URL.HostPort)
	if r.fellBackToParent {
		sv.fallbackHost = r.URL.Host
	}
	if debug {
		debug.Printf("cli(%s) connected to %s %d concurrent connections\n",
			c.RemoteAddr(), sv.hostPort, incSrvConnCnt(sv.hostPort))
	}
	return sv, nil
}

// Should call initBuf before reading http response from server. This allows
// us not init buf for connect method which does not need to parse http
// respnose.
func newServerConn(c net.Conn, hostPort string) *serverConn {
	sv := &serverConn{
		Conn:     c,
		hostPort: hostPort,
	}
	return sv
}

// Clear the pending host even on failure so a pooled HTTP connection cannot
// later learn a route for a previous target from another target's response.
func (sv *serverConn) finishFallback(success bool) {
	if sv.fallbackHost != "" {
		if success {
			domainList.add(sv.fallbackHost, domainTypeProxy)
		}
		sv.fallbackHost = ""
	}
}

func (sv *serverConn) initBuf() {
	if sv.bufRd == nil {
		sv.bufRd = httpReaderPool.Get(sv)
	}
}

func (sv *serverConn) releaseBuf() {
	if sv.bufRd != nil {
		// debug.Println("release server buffer")
		httpReaderPool.Put(sv.bufRd)
		sv.bufRd = nil
	}
}

func (sv *serverConn) Close() error {
	sv.closeOnce.Do(func() {
		sv.releaseBuf()
		if debug {
			debug.Printf("close connection to %s remains %d concurrent connections\n",
				sv.hostPort, decSrvConnCnt(sv.hostPort))
		}
		sv.closeErr = sv.closeTransport()
	})
	return sv.closeErr
}

func (sv *serverConn) closeTransport() error {
	sv.transportCloseOnce.Do(func() {
		sv.transportCloseErr = sv.Conn.Close()
	})
	return sv.transportCloseErr
}

func (sv *serverConn) Read(p []byte) (int, error) {
	if sv.readTimeout > 0 {
		if err := sv.SetReadDeadline(time.Now().Add(sv.readTimeout)); err != nil {
			return 0, err
		}
	}
	return sv.Conn.Read(p)
}

func (sv *serverConn) mayBeClosed() bool {
	if _, ok := sv.Conn.(meowConn); ok {
		debug.Println("meow parent would keep alive")
		return false
	}
	return time.Now().After(sv.willCloseOn)
}

// A CONNECT tunnel holds its copy buffer for the whole life of the connection.
// Most tunnels (a kept-alive HTTPS connection, a websocket that mostly idles)
// move little data, so they start with a small buffer; a tunnel that sustains
// full reads grows to a large buffer to cut read/write syscalls on bulk
// transfers. sync.Pool returns the excess to the runtime under memory pressure.
const (
	connectBufSmallSize = 4 * 1024
	connectBufLargeSize = 32 * 1024
	// Consecutive buffer-filling reads before growing to the large buffer.
	tunnelGrowThreshold = 4
)

var (
	connectBufSmall = newByteBufferPool(connectBufSmallSize)
	connectBufLarge = newByteBufferPool(connectBufLargeSize)
)

// tunnelBuf is an adaptive copy buffer for one direction of a CONNECT tunnel.
type tunnelBuf struct {
	buf    []byte
	large  bool
	fullRd int
}

func newTunnelBuf() *tunnelBuf { return &tunnelBuf{buf: connectBufSmall.Get()} }

func (t *tunnelBuf) release() {
	if t.buf == nil {
		return
	}
	if t.large {
		connectBufLarge.Put(t.buf)
	} else {
		connectBufSmall.Put(t.buf)
	}
	t.buf = nil
}

// observe grows the buffer once a tunnel has shown sustained throughput. Call
// it after the bytes from a read have been forwarded, never before.
func (t *tunnelBuf) observe(n int) {
	if t.large {
		return
	}
	if n < len(t.buf) {
		t.fullRd = 0
		return
	}
	if t.fullRd++; t.fullRd >= tunnelGrowThreshold {
		connectBufSmall.Put(t.buf)
		t.buf = connectBufLarge.Get()
		t.large = true
	}
}

func copyServer2Client(sv *serverConn, c *clientConn) (err error) {
	tb := newTunnelBuf()
	defer tb.release()
	buf := tb.buf

	if sv.bufRd != nil {
		buffered := sv.bufRd.Buffered()
		if buffered > 0 {
			p, peekErr := sv.bufRd.Peek(buffered)
			if peekErr != nil {
				return peekErr
			}
			if err = writeFull(c, p); err != nil {
				return err
			}
		}
		sv.releaseBuf()
	}
	for {
		// debug.Println("srv->cli")
		var n int
		n, err = sv.Read(buf)
		if n > 0 {
			sv.finishFallback(true)
			if writeErr := writeFull(c, buf[:n]); writeErr != nil {
				return writeErr
			}
			tb.observe(n)
			buf = tb.buf
		}
		if err != nil {
			// Expected error besides EOF: "use of closed network connection",
			// this is to make blocking read return.
			// debug.Printf("copyServer2Client read data: %v\n", err)
			return
		}
	}
}

func copyClient2Server(c *clientConn, sv *serverConn, r *Request) (err error) {
	var n int

	if c.bufRd != nil {
		n = c.bufRd.Buffered()
		if n > 0 {
			buffered, _ := c.bufRd.Peek(n) // should not return error
			if err = writeFull(sv, buffered); err != nil {
				// debug.Printf("cli->srv write buffered err: %v\n", err)
				return
			}
		}
		if debug {
			debug.Printf("cli(%s)->srv(%s) released read buffer\n",
				c.RemoteAddr(), r.URL.HostPort)
		}
		c.releaseBuf()
	}

	tb := newTunnelBuf()
	defer tb.release()
	for {
		// debug.Println("908: cli->srv")
		if n, err = c.Read(tb.buf); err != nil {
			debug.Printf("CONNECT client-to-upstream read stopped: %v\n", err)
			return
		}

		if err = writeFull(sv, tb.buf[:n]); err != nil {
			// XXX is it enough to only do block detection in copyServer2Client?
			debug.Printf("CONNECT client-to-upstream write stopped: %v\n", err)
			return
		}
		tb.observe(n)
		// debug.Printf("924: cli(%s)->srv(%s) sent %d bytes data\n", c.RemoteAddr(), r.URL.HostPort, n)
	}
}

// copyClient2ServerGated detects an early client disconnect while an HTTP
// upstream CONNECT response is pending. It may read one client chunk early,
// but never sends that chunk upstream until the response is successful.
func copyClient2ServerGated(c *clientConn, sv *serverConn, r *Request, established <-chan bool) (err error) {
	var pending []byte
	if c.bufRd != nil {
		buffered := c.bufRd.Buffered()
		if buffered > 0 {
			p, peekErr := c.bufRd.Peek(buffered)
			if peekErr != nil {
				return peekErr
			}
			pending = append(pending, p...)
		}
		c.releaseBuf()
	}

	tb := newTunnelBuf()
	defer tb.release()
	if len(pending) == 0 {
		n, readErr := c.Read(tb.buf)
		if readErr != nil {
			return readErr
		}
		pending = tb.buf[:n]
	}
	if ok := <-established; !ok {
		return nil
	}
	if err = writeFull(sv, pending); err != nil {
		return err
	}
	for {
		n, readErr := c.Read(tb.buf)
		if readErr != nil {
			return readErr
		}
		if err = writeFull(sv, tb.buf[:n]); err != nil {
			return err
		}
		tb.observe(n)
	}
}

var connEstablished = []byte("HTTP/1.1 200 Tunnel established\r\n\r\n")

type connectResponse struct {
	status        int
	contentLength int64
	chunked       bool
	raw           []byte
}

func readConnectResponse(sv *serverConn) (*connectResponse, error) {
	const maxConnectResponseHeader = 64 << 10
	sv.initBuf()
	raw := make([]byte, 0, 1024)
	for {
		line, err := sv.bufRd.ReadSlice('\n')
		raw = append(raw, line...)
		if len(raw) > maxConnectResponseHeader {
			return nil, errors.New("HTTP upstream CONNECT response header is too large")
		}
		if err == bufio.ErrBufferFull {
			continue
		}
		if err != nil {
			return nil, err
		}
		if len(line) == 2 && line[0] == '\r' && line[1] == '\n' {
			break
		}
	}

	lines := bytes.Split(raw, []byte(CRLF))
	fields := bytes.Fields(lines[0])
	if len(fields) < 2 || (string(fields[0]) != "HTTP/1.0" && string(fields[0]) != "HTTP/1.1") {
		return nil, fmt.Errorf("malformed HTTP upstream CONNECT response: %q", lines[0])
	}
	status, err := strconv.Atoi(string(fields[1]))
	if err != nil || status < 100 || status > 999 {
		return nil, fmt.Errorf("invalid HTTP upstream CONNECT status: %q", fields[1])
	}
	response := &connectResponse{status: status, contentLength: -1, raw: raw}
	for _, line := range lines[1:] {
		colon := bytes.IndexByte(line, ':')
		if colon <= 0 {
			continue
		}
		name := strings.ToLower(string(bytes.TrimSpace(line[:colon])))
		value := strings.TrimSpace(string(line[colon+1:]))
		switch name {
		case "content-length":
			length, parseErr := strconv.ParseInt(value, 10, 64)
			if parseErr != nil || length < 0 {
				return nil, errors.New("invalid Content-Length in HTTP upstream CONNECT response")
			}
			if response.contentLength >= 0 && response.contentLength != length {
				return nil, errors.New("conflicting Content-Length in HTTP upstream CONNECT response")
			}
			response.contentLength = length
		case "transfer-encoding":
			for _, encoding := range strings.Split(strings.ToLower(value), ",") {
				if strings.TrimSpace(encoding) == "chunked" {
					response.chunked = true
				}
			}
		}
	}
	if response.chunked && response.contentLength >= 0 {
		return nil, errors.New("HTTP upstream CONNECT response has both Transfer-Encoding and Content-Length")
	}
	return response, nil
}

func forwardRejectedConnectBody(c *clientConn, sv *serverConn, response *connectResponse) error {
	switch {
	case response.status == 204 || response.status == 304:
		return nil
	case response.chunked:
		return sendBodyChunked(fullWriter{c}, sv.bufRd, httpBufSize)
	case response.contentLength >= 0:
		_, err := io.CopyN(fullWriter{c}, sv.bufRd, response.contentLength)
		return err
	default:
		return copyServer2Client(sv, c)
	}
}

func (sv *serverConn) establishHTTPConnect(r *Request, c *clientConn) error {
	sv.readTimeout = config.ReadTimeout
	defer func() {
		sv.readTimeout = 0
		_ = sv.SetReadDeadline(zeroTime)
	}()
	for interim := 0; interim < 5; interim++ {
		response, err := readConnectResponse(sv)
		if err != nil {
			return err
		}
		if err = writeFull(c, response.raw); err != nil {
			return err
		}
		if response.status >= 200 && response.status < 300 {
			sv.finishFallback(true)
			return nil
		}
		if response.status >= 100 && response.status < 200 && response.status != 101 {
			continue
		}
		sv.finishFallback(false)
		if err = forwardRejectedConnectBody(c, sv, response); err != nil && err != io.EOF {
			return err
		}
		return fmt.Errorf("HTTP upstream rejected CONNECT with status %d", response.status)
	}
	return errors.New("too many interim HTTP upstream CONNECT responses")
}

// Do HTTP CONNECT
func (sv *serverConn) doConnect(r *Request, c *clientConn) (err error) {
	started := time.Now()
	defer sv.Close()
	defer func() {
		if debug {
			debug.Printf("CONNECT %s finished after %s: %v\n", r.URL.HostPort, time.Since(started), err)
		}
	}()
	r.state = rsCreated

	_, isHttpConn := sv.Conn.(httpConn)
	_, isHttpsConn := sv.Conn.(httpsConn)
	_, isMeowConn := sv.Conn.(meowConn)
	isHTTPParent := isHttpConn || isHttpsConn || isMeowConn
	cli2srvDone := make(chan error, 1)
	if isHTTPParent {
		if debug {
			debug.Printf("cli(%s) send CONNECT request to parent\n", c.RemoteAddr())
		}
		if err = sv.sendHTTPProxyRequestHeader(r); err != nil {
			debug.Printf("cli(%s) error send CONNECT request to parent: %v\n",
				c.RemoteAddr(), err)
			return err
		}
		established := make(chan bool, 1)
		go func() {
			copyErr := copyClient2ServerGated(c, sv, r, established)
			sv.closeTransport()
			cli2srvDone <- copyErr
		}()
		if err = sv.establishHTTPConnect(r, c); err != nil {
			established <- false
			debug.Printf("cli(%s) HTTP upstream CONNECT failed: %v\n", c.RemoteAddr(), err)
			sv.Close()
			_ = c.Conn.Close()
			<-cli2srvDone
			return err
		}
		established <- true
	} else {
		// debug.Printf("send connection confirmation to %s->%s\n", c.RemoteAddr(), r.URL.HostPort)
		if err = writeFull(c, connEstablished); err != nil {
			debug.Printf("cli(%s) error send 200 Connecion established: %v\n",
				c.RemoteAddr(), err)
			return err
		}
		go func() {
			copyErr := copyClient2Server(c, sv, r)
			sv.closeTransport()
			cli2srvDone <- copyErr
		}()
	}
	r.releaseBuf()

	// debug.Printf("doConnect: srv(%s)->cli(%s)\n", r.URL.HostPort, c.RemoteAddr())
	err = copyServer2Client(sv, c)
	// Close the client connection to force a blocked client-to-server read to
	// return, then wait until that goroutine has closed the server connection.
	c.Conn.Close()
	cli2srvErr := <-cli2srvDone
	if cli2srvErr != nil {
		return cli2srvErr
	}
	return
}

func (sv *serverConn) sendHTTPProxyRequestHeader(r *Request) (err error) {
	if err = writeFull(sv, r.proxyRequestLine()); err != nil {
		return err
	}
	if hc, ok := sv.Conn.(httpConn); ok && hc.parent.authHeader != nil {
		// Add authorization header for parent http proxy
		if err = writeFull(sv, hc.parent.authHeader); err != nil {
			return err
		}
	} else if hc, ok := sv.Conn.(httpsConn); ok && hc.parent.authHeader != nil {
		// Add authorization header for parent http proxy
		if err = writeFull(sv, hc.parent.authHeader); err != nil {
			return err
		}
	}
	if err = writeFull(sv, r.rawHeaderBody()); err != nil {
		return err
	}
	return
}

func (sv *serverConn) sendRequestHeader(r *Request) (err error) {
	// Send request to the server
	switch sv.Conn.(type) {
	case httpConn, httpsConn, meowConn:
		return sv.sendHTTPProxyRequestHeader(r)
	}
	if err = writeFull(sv, r.rawRequest()); err != nil {
		return err
	}
	return
}

func (sv *serverConn) sendRequestBody(r *Request, c *clientConn) (err error) {
	if !r.hasBody() {
		return
	}

	err = sendBody(fullWriter{sv}, c.bufRd, r.ContLen, r.Chunking)
	if err != nil {
		errl.Printf("cli(%s) send request body error %v %s\n", c.RemoteAddr(), err, r)
		return
	}
	if debug {
		debug.Printf("cli(%s) request body sent %s\n", c.RemoteAddr(), r)
	}
	return
}

// Do HTTP request other that CONNECT
func (sv *serverConn) doRequest(c *clientConn, r *Request, rp *Response) (err error) {
	r.state = rsCreated
	if err = sv.sendRequestHeader(r); err != nil {
		return
	}
	if r.ExpectContinue && r.hasBody() {
		if err = c.relayExpectContinue(sv, r, rp); err != nil {
			if err == errExpectRejected {
				r.state = rsSent
			}
			return
		}
	}
	if err = sv.sendRequestBody(r, c); err != nil {
		return
	}
	r.state = rsSent
	return c.readResponse(sv, r, rp)
}

// Send response body if header specifies content length
func sendBodyWithContLen(w io.Writer, r *bufio.Reader, contLen int64) (err error) {
	// debug.Println("Sending body with content length", contLen)
	if contLen == 0 {
		return
	}
	if err = copyN(w, r, contLen, httpBufSize); err != nil {
		debug.Println("sendBodyWithContLen error:", err)
	}
	return
}

// Use this function until we find Trailer headers actually in use.
func skipTrailer(r *bufio.Reader) error {
	// It's possible to get trailer headers, but the body will always end with
	// a line with just CRLF.
	for {
		s, err := r.ReadSlice('\n')
		if err != nil {
			errl.Println("skip trailer:", err)
			return err
		}
		if len(s) == 2 && s[0] == '\r' && s[1] == '\n' {
			return nil
		}
		errl.Printf("skip trailer: %#v", string(s))
		if len(s) == 1 || len(s) == 2 {
			return fmt.Errorf("malformed chunk body end: %#v", string(s))
		}
	}
}

func skipCRLF(r *bufio.Reader) (err error) {
	var buf [2]byte
	if _, err = io.ReadFull(r, buf[:]); err != nil {
		errl.Println("skip chunk body end:", err)
		return
	}
	if buf[0] != '\r' || buf[1] != '\n' {
		return fmt.Errorf("malformed chunk body end: %#v", string(buf[:]))
	}
	return
}

// Send response body if header specifies chunked encoding. rdSize specifies
// the size of each read on Reader, it should be set to be the buffer size of
// the Reader, this parameter is added for testing.
func sendBodyChunked(w io.Writer, r *bufio.Reader, rdSize int) (err error) {
	// debug.Println("Sending chunked body")
	for {
		var s []byte
		// Read chunk size line, ignore chunk extension if any.
		if s, err = r.ReadSlice('\n'); err != nil {
			errl.Println("read chunk size:", err)
			return
		}
		smid := bytes.IndexByte(s, ';')
		if smid == -1 {
			smid = len(s)
		} else {
			// use error log to find usage of chunk extension
			errl.Printf("got chunk extension: %s\n", s)
		}
		var size int64
		if size, err = ParseIntFromBytes(TrimSpace(s[:smid]), 16); err != nil {
			errl.Println("chunk size invalid:", err)
			return
		}
		if size == 0 {
			if err = skipCRLF(r); err != nil {
				return
			}
			if err = writeFull(w, []byte(chunkEnd)); err != nil {
				debug.Println("send chunk ending:", err)
			}
			return
		}
		// RFC 2616 19.3 only suggest tolerating single LF for
		// headers, not for chunked encoding. So assume the server will send
		// CRLF. If not, the following parse int may find errors.
		if err = writeFull(w, s); err != nil {
			return
		}
		if size > math.MaxInt64-2 {
			return errors.New("chunk size too large")
		}
		if err = copyN(w, r, size+2, rdSize); err != nil {
			debug.Println("copy chunked data:", err)
			return
		}
	}
}

const chunkEnd = "0\r\n\r\n"

func sendBodySplitIntoChunk(w io.Writer, r *bufio.Reader) (err error) {
	// debug.Printf("sendBodySplitIntoChunk called\n")
	for {
		if _, err = r.Peek(1); err != nil {
			if errors.Is(err, io.EOF) {
				// EOF is expected here as the server is closing connection.
				// debug.Println("end chunked encoding")
				err = writeFull(w, []byte(chunkEnd))
				if err != nil {
					debug.Println("write chunk end 0", err)
				}
				return
			}
			debug.Println("read error in sendBodySplitIntoChunk", err)
			return
		}
		buffered := r.Buffered()
		b, peekErr := r.Peek(buffered)
		if peekErr != nil {
			return peekErr
		}

		chunkSize := []byte(fmt.Sprintf("%x\r\n", len(b)))
		if err = writeFull(w, chunkSize); err != nil {
			debug.Printf("write chunk size %v\n", err)
			return
		}
		if err = writeFull(w, b); err != nil {
			debug.Println("write chunk data:", err)
			return
		}
		if _, err = r.Discard(buffered); err != nil {
			return err
		}
		if err = writeFull(w, []byte(CRLF)); err != nil {
			debug.Println("write chunk ending CRLF:", err)
			return
		}
	}
}

// Send message body.
func sendBody(w io.Writer, bufRd *bufio.Reader, contLen int64, chunk bool) (err error) {
	// chunked encoding has precedence over content length
	// meow does not sanitize response header, but can correctly handle it
	if chunk {
		err = sendBodyChunked(w, bufRd, httpBufSize)
	} else if contLen >= 0 {
		// It's possible to have content length 0 if server response has no
		// body.
		err = sendBodyWithContLen(w, bufRd, contLen)
	} else {
		// Must be reading server response here, because sendBody is called in
		// reading response iff chunked or content length > 0.
		err = sendBodySplitIntoChunk(w, bufRd)
	}
	return
}
