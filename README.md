...
判定、聚合后的上游错误、父代理健康、连接池和 goroutine 数；状态页每 5 秒刷新且
仅允许本机访问。页面里每条分流记录都有「忘记」链接，可让该主机名下次重新判定；
顶部的「重新加载全部配置」会就地重载 direct/proxy/reject 及中国 IP 列表并清空自动
学习结果；若 rc 本身也有改动，会先在子进程里校验，通过后自动重启以生效（rc 有
语法错误时会指出出错位置并继续使用当前配置，不重启）。Windows 上重启改为退出，
需手动再启动。
